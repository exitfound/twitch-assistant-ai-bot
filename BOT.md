...
- `_cooldowns = {}` — словарь `{username: timestamp}` для per-user кулдаунов
...
Проверяется in-memory словарь `_cooldowns` (сбрасывается при перезапуске). Если прошло меньше `COOLDOWN_SECONDS` — бот отвечает оставшимся временем и выходит.
...
Проверяются в порядке: `!help` → `!stat` → `!top` → проверка прав для `!fact`/`!defact` → `!defact` → `!fact` → `!ask`.

Все команды выходят до вызова Gemini с контекстом. Кулдаун на них **не ставится** (кроме `!ask`, который ставит свой).
...
Хранит пары вопрос-ответ. Отдельная таблица — бот не видит свои прошлые ответы в контексте, что предотвращает зацикливание. Индекс на `session_id`. Используется для `!stat` и `!top`. Проактивные сообщения сохраняются с `_proactive_` как username, `!ask` — с `[ask]` префиксом в `user_message`, follow — с `[follow]`.
...
- `get_session_stats` / `get_session_top_users` — статистика текущей сессии
