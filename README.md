...
Twitch-бот `securityexpert` для канала `exitfound`. Генерирует ответы через **Gemini 2.5 Flash**. Хранит полную историю чата в SQLite с полнотекстовым поиском (FTS5) и долговременной памятью между стримами.
...
Зарегистрируй отдельный Twitch-аккаунт, который будет выступать ботом (например `securityexpert`). Это обычный аккаунт — специальной регистрации «как бот» не требуется.

После создания аккаунта зайди в чат стримерского канала (под стримерским аккаунтом) и дай боту права модератора:

```
/mod botname
```

Модератор нужен чтобы бот мог писать в чат без ограничений Twitch по rate-limit. Делается один раз — разрешение хранится на стороне Twitch и сохраняется при переносе бота на другую машину.
...
Зайди на [aistudio.google.com](https://aistudio.google.com/) → **Get API key** → создай ключ. Бесплатного тарифа достаточно для работы бота в чате.
...
Открой эту ссылку в браузере. Важно: залогинься именно **под бот-аккаунтом**, не под стримерским. Разреши запрошенные права. После успешной авторизации twitchio перехватит callback и в консоли появятся токены:
...
```
Bot started | Username: securityexpert | Session: 2026-03-21

Subscribed to chat #exitfound
...
Команды работают через те же триггеры. Примеры: `@securityexpert !stat`, `сосурян !help`, реплай на бота с `!fact я из Питера`.
...
| `!top` | Показывает топ-5 пользователей по обращениям к боту за сессию и за всё время |
| Любой другой текст | Отправляется как вопрос в Gemini вместе с контекстом |

### Кулдаун

Каждый пользователь может обращаться к боту не чаще одного раза в N секунд (по умолчанию 10). Кулдаун устанавливается **до** вызова Gemini API — это предотвращает дублирующие ответы. При срабатывании кулдауна бот отвечает сообщением с оставшимся временем ожидания.

### Обработка ответа

1. **Удаление `@username`** — если Gemini начинает ответ с `@username:` или `@username,`, эта часть срезается. Затем все оставшиеся `@username` адресата удаляются. Упоминания других пользователей не затрагиваются.
2. **Обрезка до 450 символов** — лимит Twitch — 500 символов, 450 оставляет запас для `@username: `.
3. **CAPS-режим** — если исходное сообщение написано КАПСОМ (85%+ букв) или с вероятностью 30%, ответ переводится в CAPS. При этом `@упоминания` остаются в оригинальном регистре.
4. **Отправка** — ответ отправляется как `@username: <текст>`.
5. **Сохранение** — пара «вопрос → ответ» сохраняется в `bot_interactions`.
...
Если Gemini возвращает пустой ответ (вероятная блокировка входным фильтром), бот делает повторный запрос без секций `[Язык чата]` и `[Контекст канала]`. Если повторный запрос тоже пустой — бот отвечает «не удалось получить ответ».
...
Загрузка лора из txt файлов (одна запись на строку, `#` — комментарии). Подробности формата — в [BOT.md](BOT.md).

```bash
./venv/bin/python3 bot.py --upload-lore lore.txt
...
1. Клонируй репо, создай venv, установи зависимости
2. Скопируй `.env` с прежней машины
3. Если нужна история — скопируй `chat_history.db`, `chat_history.db-wal` и `chat_history.db-shm`
4. `/mod` хранится на стороне Twitch — повторно давать не нужно
5. `TWITCH_BOT_TOKEN` авто-обновляется через `TWITCH_BOT_REFRESH`
