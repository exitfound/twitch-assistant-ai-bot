...
from src.database import (
    ...
    get_session_top_users, get_total_top_users,
)
...
class ChatComponent(commands.Component):
    ...
    async def event_message(self, message: twitchio.ChatMessage):
        ...
        if user in self.bot._cooldowns:
            remaining = Cooldown.SECONDS - (now - self.bot._cooldowns[user])
            if remaining > 0:
        ...
        if prompt == TOP_TRIGGER:
            session_top, total_top = await asyncio.gather(
                get_session_top_users(self.bot.session_id),
                get_total_top_users(),
            )
            parts = []
            if session_top:
                top_str = ', '.join(f'{i}. {u} ({c})' for i, (u, c) in enumerate(session_top, 1))
                parts.append(f'сессия: {top_str}')
            if total_top:
                top_str = ', '.join(f'{i}. {u} ({c})' for i, (u, c) in enumerate(total_top, 1))
                parts.append(f'всего: {top_str}')
            await message.respond(f'@{user}: {" | ".join(parts)}' if parts else f'@{user}: пока нет данных.')
            return
        ...
        if prompt.startswith(ASK_TRIGGER):
            ...
            if ask_prompt:
                self.bot._cooldowns[user] = time.time()
                try:
