...
async def test_offline_redemption_is_refunded(db):
    ...
    assert await handle_redemption(bot, game.ACTION_SHIELD, 'r1', 'gop', '') is False
...
async def test_successful_reward_is_fulfilled(db):
    bot = FakeBot()
    assert await handle_redemption(bot, game.ACTION_SHIELD, 'r1', 'gop', '') is True
...
async def test_refused_reward_is_refunded_with_its_reason(db):
    bot = FakeBot()
    assert await handle_redemption(bot, game.ACTION_REROLL, 'r1', 'gop', 'gop') is False
...
async def test_duplicate_is_left_alone_silently(db):
    bot = FakeBot()
    await handle_redemption(bot, game.ACTION_SHIELD, 'r1', 'gop', '')
    bot.send_chat_message.reset_mock()
    assert await handle_redemption(bot, game.ACTION_SHIELD, 'r1', 'gop', '') is None
...
async def test_first_reroll_of_a_player_uses_its_own_text(db):
    ...
    await handle_redemption(bot, game.ACTION_REROLL, 'r1', 'gop', 'victim')
...
async def test_game_error_refunds(db, monkeypatch):
    ...
    bot = FakeBot()
    assert await handle_redemption(bot, game.ACTION_EXTRA, 'r1', 'gop', '') is False
...
async def test_failing_save_still_settles_the_redemption(db, monkeypatch):
    ...
    assert await handle_redemption(FakeBot(), game.ACTION_SHIELD, 'r1', 'gop', '') is True
