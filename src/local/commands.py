...
async def handle_stats(ctx: CommandContext) -> None:
    ...
    # Answer order: personal, then the stream, then the channel all-time. Each part is
    # its own key rather than a placeholder in a shared text, because CONTENT.md is
    # re-read live and a new placeholder in an old key reaches chat as a raw template
...
def _session_date(session_id: str) -> str:
    """The session day as DD.MM.YYYY.

    A session id is «YYYY-MM-DD HH:MM» for a stream and «YYYY-MM-DD» outside one. Chat
    needs only the day: the stream start time tells a viewer nothing.
    """
